...
# ===================== ÓSCAR: carga desde Oscar_Data_1927_today.xlsx =====================

@st.cache_data
def load_oscar_data_from_excel(path_xlsx="Oscar_Data_1927_today.xlsx"):
    """
    Carga robusta del Excel 'Oscar_Data_1927_today.xlsx' y normaliza
    las columnas que usa la sección de Premios Óscar.

    Salida garantizada:
      - YearInt        (año del premio / ceremonia)
      - CanonCat       (categoría canónica)
      - Film           (película)
      - Nominee        (persona/entidad nominada)
      - NomineeIdsList (lista de IDs o vacío)
      - IsWinner       (True si es ganador)
      - NormFilm       (título normalizado para cruzar con tu catálogo)
    """
    try:
        raw = pd.read_excel(path_xlsx, engine="openpyxl")
    except Exception as e:
        st.error(f"No se pudo cargar '{path_xlsx}': {e}")
        return pd.DataFrame()
    ...
    # -------- Año del premio / ceremonia -> YearInt --------
    year_candidates = [
        "Year", "Ceremony Year", "Award Year", "Year_of_Award",
        "Year_film", "Year_Film"
    ]
    year_series = None
    ...
    if year_series is None:
        year_series = pd.Series([-1] * len(df), index=idx)

    yr = pd.to_numeric(year_series, errors="coerce")
    ...
    if "CanonicalCategory" in df.columns:
        ...
        cat_candidates = ["CanonCat", "Category", "Category Name", "Award", "Award Category"]
        found = None
        for c in cat_candidates:
            if c in df.columns:
                ...
                break
        if found:
            df["CanonCat"] = df[found].astype(str)
        else:
            df["CanonCat"] = ""

    # -------- Ganador -> IsWinner --------
    win_candidates = ["IsWinner", "Winner", "Winner Flag", "WinnerFlag", "Won?"]
    w = None
    for c in win_candidates:
        if c in df.columns:
            w = df[c].astype(str).str.lower()
            break
    if w is None:
        w = pd.Series([""] * len(df), index=idx)

    df["IsWinner"] = w.isin(
        ["1", "true", "yes", "winner", "win", "won", "y", "x", "si", "sí"]
    )

    # -------- Nominee (persona / entidad) --------
    nom_candidates = ["Nominee", "Nominee Name", "Name", "Person", "Entity"]
    found_nom = None
    for c in nom_candidates:
        if c in df.columns:
            ...
            break
    if found_nom:
        df["Nominee"] = df[found_nom].astype(str)
    else:
        df["Nominee"] = ""

    # -------- Film (título de la película) --------
    film_candidates = ["Film", "Film Title", "Film_Title", "Title", "Picture", "Movie"]
    found_film = None
    for c in film_candidates:
        if c in df.columns:
            found_film = c
            break
    if found_film:
        df["Film"] = df[found_film].astype(str)
    else:
        df["Film"] = ""

    # -------- NomineeIdsList (IDs de entidades, si existen) --------
    if "NomineeIds" in df.columns:
        ids_raw = df["NomineeIds"].fillna("").astype(str)
    elif "EntityIds" in df.columns:
        ids_raw = df["EntityIds"].fillna("").astype(str)
    else:
        ids_raw = pd.Series([""] * len(df), index=idx)

    df["NomineeIdsList"] = ids_raw.apply(
        lambda s: [x.strip() for x in re.split(r"[;,]", s) if x.strip()]
    )

    # -------- Título normalizado para cruce con catálogo --------
...
# ============================================================
#           TAB 4: PREMIOS ÓSCAR (USANDO EXCEL)
# ============================================================

# ----------------- Helpers Óscar: carga desde Excel -----------------

@st.cache_data
def load_oscar_data_from_excel(path_xlsx="Oscar_Data_1927_today.xlsx"):
    """
    Carga robusta del Excel Oscar_Data_1927_today.xlsx y lo normaliza a un esquema:
    - YearFilmInt: año de la película (base para los botones de año)
    - YearCeremonyInt: año de la ceremonia (si existe)
    - CategoryNorm: categoría "bonita"
    - Film, Nominee, NomineeIds
    - IsWinner: bool
    - NormFilm: título normalizado para cruzar con tu catálogo
    """
    try:
    ...
    def col_or_empty(name, default=""):
        return raw[name] if name in raw.columns else pd.Series([default] * len(raw), index=idx)

    # ---- Año de película (YearFilmInt) ----
    year_film_col = None
    for cand in ["Year_Film", "Year Film", "Year of Film", "Film_Year", "Film Year", "Year"]:
        if cand in raw.columns:
            year_film_col = cand
            break
    if year_film_col is None:
        # último recurso: cualquier columna que parezca año
        for c in raw.columns:
            if "year" in c.lower():
                year_film_col = c
                break

    if year_film_col is not None:
        yf = pd.to_numeric(raw[year_film_col], errors="coerce")
        raw["YearFilmInt"] = yf.fillna(-1).astype(int)
    else:
        raw["YearFilmInt"] = -1

    # ---- Año de ceremonia (YearCeremonyInt) ----
    year_cer_col = None
    for cand in ["Year_Ceremony", "Year Ceremony", "Ceremony_Year", "Ceremony Year", "Ceremony"]:
        if cand in raw.columns:
            year_cer_col = cand
            break

    if year_cer_col is not None:
        yc = pd.to_numeric(raw[year_cer_col], errors="coerce")
        raw["YearCeremonyInt"] = yc.fillna(-1).astype(int)
    else:
        # si no hay, uso el de película
        raw["YearCeremonyInt"] = raw["YearFilmInt"]

    # ---- Categoría normalizada ----
    cat_col = None
    for cand in ["CanonicalCategory", "Canonical_Category", "CanonCategory", "CanonCat", "Category"]:
        if cand in raw.columns:
            cat_col = cand
            break
    if cat_col is None:
        cat_col = raw.columns[0]  # lo que haya, pero algo

    raw["CategoryNorm"] = col_or_empty(cat_col).astype(str).str.strip()

    # ---- Texto de nominado / película / IDs ----
    raw["Nominee"] = col_or_empty("Nominee").astype(str)
    raw["Film"] = col_or_empty("Film").astype(str)
    ids_base = col_or_empty("NomineeIds").astype(str).fillna("")
    raw["NomineeIdsList"] = ids_base.apply(
        lambda s: [x.strip() for x in re.split(r"[;,]", s) if x.strip()]
    )

    # ---- Ganador (IsWinner) robusto ----
    wcol = None
    for cand in ["IsWinner", "Winner", "Winner?", "winner", "WINNER", "is_winner"]:
        if cand in raw.columns:
            wcol = cand
            break
    ...
    if wcol is not None:
        ...
        if pd.api.types.is_numeric_dtype(col):
            tmp = pd.to_numeric(col, errors="coerce").fillna(0)
            raw["IsWinnerNorm"] = tmp > 0.5
        else:
            s = col.astype(str).str.strip().str.lower()
            raw["IsWinnerNorm"] = (
                s.isin(["1", "yes", "y", "true", "winner", "win", "won", "sí", "si"])
                | s.str.contains("winner", na=False)
            )
    else:
        raw["IsWinnerNorm"] = False

    # ---- Título normalizado para enlace con tu catálogo ----
    raw["NormFilm"] = raw["Film"].apply(normalize_title)

    # Devuelvo un DataFrame limpio
    out = pd.DataFrame({
        "YearFilmInt": raw["YearFilmInt"],
        "YearCeremonyInt": raw["YearCeremonyInt"],
        "CategoryNorm": raw["CategoryNorm"],
        "Nominee": raw["Nominee"],
        "Film": raw["Film"],
        "NomineeIdsList": raw["NomineeIdsList"],
        "IsWinner": raw["IsWinnerNorm"].fillna(False),
        "NormFilm": raw["NormFilm"],
    })

    return out


def render_year_chips(all_years, key_prefix="osc_years"):
    """
    Muestra los años en filas horizontales de 'chips' y devuelve el año seleccionado.
    Usa st.session_state[f"{key_prefix}_selected"] para recordar la selección.
    """
    if not all_years:
        return None

    all_years = sorted(int(y) for y in all_years if y > 0)
    if not all_years:
        return None

    state_key = f"{key_prefix}_selected"
    if state_key not in st.session_state:
        st.session_state[state_key] = all_years[-1]  # último año por defecto

    selected = st.session_state[state_key]

    # 14 años por fila aprox
    per_row = 14
    for i in range(0, len(all_years), per_row):
        row_years = all_years[i:i + per_row]
        cols = st.columns(len(row_years))
        for col, y in zip(cols, row_years):
            with col:
                is_sel = (y == selected)
                base_style = (
                ...
                if is_sel:
                    style = (
                        base_style +
                        "background: radial-gradient(circle at top left, rgba(234,179,8,0.45), rgba(15,23,42,1)); "
                        "border: 1px solid #facc15; color:#fefce8; "
                ...
                clicked = st.button(
                    str(y),
                    key=f"{key_prefix}_{y}",
                    help=f"Año de película {y}",
                )
                # hack visual: aplico estilo via markdown (el button de Streamlit no deja CSS directo)
                st.markdown(
                    f"<style>div[data-testid='stButton'] button[kind='secondary']#{key_prefix}_{y} {{ {style} }}</style>",
                    unsafe_allow_html=True,
                )
...
def load_oscar_data_from_excel(path_xlsx="Oscar_Data_1927_today.xlsx"):
    """
    Carga robusta del Excel Oscar_Data_1927_today.xlsx y lo normaliza a un esquema:
    - YearFilmInt: año de la película (base para los años)
    - YearCeremonyInt: año de la ceremonia (si existe)
    - CategoryNorm: categoría "bonita"
    - Film, Nominee, NomineeIdsList
    - IsWinner: bool (interpretando distintas variantes)
    - NormFilm: título normalizado para cruzar con tu catálogo
    """
    try:
        raw = pd.read_excel(path_xlsx, engine="openpyxl")
    ...
    if wcol is not None:
        ...
        if pd.api.types.is_numeric_dtype(col):
            ...
            s = col.astype(str).str.strip().str.lower()
            raw["IsWinnerNorm"] = (
                s.isin(["1", "yes", "y", "true", "winner", "win", "won", "si", "sí"])
                | s.str.contains("winner", na=False)
            )
    else:
        raw["IsWinnerNorm"] = False

    # ---- Título normalizado para catálogo ----
    raw["NormFilm"] = raw["Film"].apply(normalize_title)
...
with tab_awards:
    ...
    try:
        ...
        st.error(
            "No se pudo cargar **Oscar_Data_1927_today.xlsx**.\n\n"
            "Verifica que el archivo esté en la raíz del repo."
        )
        st.stop()

    if osc is None or osc.empty:
        st.error("El archivo **Oscar_Data_1927_today.xlsx** está vacío o no se pudo procesar.")
        st.stop()

    # Enlazar con tu catálogo (usa YearFilmInt como YearInt)
    ...
    with colf1:
        all_cats = sorted(osc_x["CategoryNorm"].dropna().unique().tolist())
    ...
    with colf2:
        q_aw = st.text_input(
            "Buscar (categoría / película / persona)",
            placeholder="Ej: 'Best Picture' o 'Nolan' o 'Paramount'",
            key="osc_search",
        )

    # Filtro base por año
    ff_base = osc_x[osc_x["YearFilmInt"] == year_selected].copy()

    if cats_sel:
        ff_base = ff_base[ff_base["CategoryNorm"].isin(cats_sel)]

    if q_aw:
        q = q_aw.strip().lower()
        ff_base = ff_base[
            ff_base["CategoryNorm"].astype(str).str.lower().str.contains(q, na=False)
            | ff_base["Nominee"].astype(str).str.lower().str.contains(q, na=False)
            | ff_base["Film"].astype(str).str.lower().str.contains(q, na=False)
        ]
    ...
    with c1:
        st.metric("AÑO SELECCIONADO", f"{year_selected}")
    with c2:
        st.metric("FILAS (NOMINACIONES + GANADORES)", len(ff_base))
    with c3:
        st.metric("CATEGORÍAS", ff_base["CategoryNorm"].nunique())
    with c4:
        st.metric("GANADORES (FILTRADOS)", int(ff_base["IsWinner"].sum()))

    st.caption(
        "Datos desde **Oscar_Data_1927_today.xlsx**. "
        "El borde verde marca las películas ganadoras. "
        "Los chips dorados indican si la película está en tu catálogo."
    )

    # ============================================================
    #                HELPERS PARA TARJETAS / FICHAS
    # ============================================================

    def build_detail_html(row, title_display, year_film):
        """Ficha tipo catálogo: IMDb, link, reseñas, etc."""
        lines = []

        imdb_rating = row.get("MyIMDb")
        my_rating = row.get("MyRating")
        url = row.get("CatalogURL")

        if pd.notna(imdb_rating):
            lines.append(f"IMDb: {float(imdb_rating):.1f}")

        if isinstance(url, str) and url.startswith("http"):
            lines.append(
                f"<a href='{url}' target='_blank'>Ver en mi ficha de IMDb</a>"
            )

        # Reseñas en español (Google)
        reseñas_url = get_spanish_review_link(title_display, year_film)
        if reseñas_url:
            lines.append(
                f"<a href='{reseñas_url}' target='_blank'>Reseñas en español</a>"
            )
        ...
        # Render como bloques
        detail_html = "".join(
            f"<div style='margin:0 0 2px 0;'>{line}</div>" for line in lines
        )

        # Badge catálogo
        in_catalog = bool(row.get("InMyCatalog", False))
        badge_parts = []
        if in_catalog:
            extra = (
                f" · Mi nota: {float(my_rating):.1f}"
                if pd.notna(my_rating)
                else ""
            )
            badge_parts.append(
                "<span style='background:rgba(234,179,8,0.16);"
                "border-radius:999px;padding:2px 8px;font-size:0.7rem;"
                "text-transform:uppercase;letter-spacing:0.12em;"
                "border:1px solid #facc15;color:#fef9c3;'>"
                f"En mi catálogo{extra}</span>"
            )

        return detail_html, "".join(badge_parts)

    def get_poster_html(title_display, year_film):
        poster_url = None
        if use_tmdb_gallery and title_display:
            info = get_tmdb_basic_info(title_display, year_film)
            if info:
        ...
        if poster_url:
            return (
                "<div class='movie-poster-frame'>"
                f"<img src='{poster_url}' alt='{title_display}' class='movie-poster-img' />"
                "</div>"
            )
        else:
            return (
    ...
    # ============================================================
    #    GALERÍA VISUAL — NOMINADOS + MODO "SÓLO GANADORAS"
    # ============================================================

    st.markdown("### 🖼️ Galería visual por categoría")

    show_winners_only = st.checkbox(
        "🎚️ Mostrar sólo las películas ganadoras",
        value=False,
        key="osc_winners_only",
    )

    if ff_base.empty:
        st.info("No hay nominaciones para ese año con los filtros actuales.")
    else:
        # ---------- MODO SÓLO GANADORAS: GRID POR PELÍCULA ----------
        if show_winners_only:
            st.caption("Mostrando **sólo películas ganadoras** en el año seleccionado.")

            winners = ff_base[ff_base["IsWinner"] == True].copy()
            if winners.empty:
                st.info("No hay ganadoras para este año con los filtros actuales.")
            else:
                # Agrupar por película y año → sumar categorías ganadas y personas
                grp = winners.groupby(["Film", "YearFilmInt"])

                cards_html_parts = ['<div class="movie-gallery-grid">']

                for (film, yfilm), g in grp:
                    film = (film or "").strip()
                    if not film:
                        continue

                    year_film = yfilm if not pd.isna(yfilm) and int(yfilm) > 0 else year_selected
                    title_display = film
                    year_str = f" ({int(year_film)})" if year_film else ""

                    # Poster
                    poster_html = get_poster_html(title_display, year_film)

                    # Ficha tipo catálogo (IMDb, link, reseñas, badge catálogo)
                    # Tomamos la primera fila como representante para los campos de catálogo
                    base_row = g.iloc[0]
                    detail_html, catalog_badge_html = build_detail_html(
                        base_row, title_display, year_film
                    )

                    # Lista de premios (categoría + persona)
                    cat_lines = []
                    for _, r in g.iterrows():
                        cat = (r.get("CategoryNorm") or "").strip()
                        person = (r.get("Nominee") or "").strip()
                        if person:
                            cat_lines.append(f"{cat}: <b>{person}</b>")
                        else:
                            cat_lines.append(cat)

                    awards_list_html = "".join(
                        f"<li>{line}</li>" for line in cat_lines
                    )

                    # Borde y glow (todas aquí son ganadoras)
                    border_color = "#22c55e"
                    ...
                    card_html = (
                        ...
                        "<b>Premios en este año:</b>"
                        f"<ul style='margin:4px 0 0 18px;padding:0;font-size:0.78rem;'>{awards_list_html}</ul>"
                        "</div>"
        ...
        # ---------- MODO COMPLETO: TODAS LAS CATEGORÍAS Y NOMINADOS ----------
        else:
            st.caption(
                "Mostrando **todas las nominaciones** del año, agrupadas por categoría. "
                "Verde = película ganadora; dorado = está en tu catálogo."
            )

            cats_in_year = sorted(ff_base["CategoryNorm"].dropna().unique().tolist())

            for cat in cats_in_year:
                sub = ff_base[ff_base["CategoryNorm"] == cat].copy()
                if sub.empty:
                    continue
                ...
                for _, row in sub.iterrows():
                    ...
                    in_catalog = bool(row.get("InMyCatalog", False))
                    year_film = row.get("YearFilmInt", year_selected)

                    if film:
                        title_display = film
                    elif nominee:
                        title_display = nominee
                    else:
                        title_display = "Sin título"

                    year_str = (
                        f" ({int(year_film)})"
                        if not pd.isna(year_film) and int(year_film) > 0
                        else ""
                    )
                    ...
                    # Ficha tipo catálogo
                    detail_html, catalog_badge_html = build_detail_html(
                        row, title_display, year_film
                    )

                    # Texto de persona nominada (actor, director foto, etc.)
                    person_html = ""
                    if nominee and nominee.lower() not in film.lower():
                        person_html = (
                            f"<div style='margin-top:4px;font-size:0.78rem;'>"
                            f"<b>Persona nominada:</b> {nominee}</div>"
                        )

                    # Estilo según ganador / catálogo
                    if is_winner:
                        border_color = "#22c55e"
                        glow_color = "rgba(34,197,94,0.65)"
                    elif in_catalog:
                        border_color = "#eab308"
                        glow_color = "rgba(234,179,8,0.55)"
                    else:
                    ...
                    if catalog_badge_html:
                        badge_parts.append(catalog_badge_html)
    ...
    if ff_base.empty:
        ...
        table_year = ff_base.copy().sort_values(
            ["CategoryNorm", "IsWinner", "Film", "Nominee"],
        ...
        pretty["En mi catálogo"] = pretty["En mi catálogo"].map(
            {True: "✅", False: "—"}
        )
        ...
        if "Mi nota" in pretty.columns:
            pretty["Mi nota"] = pretty["Mi nota"].apply(fmt_rating)
        if "IMDb" in pretty.columns:
            pretty["IMDb"] = pretty["IMDb"].apply(fmt_rating)
    ...
    # ============================================================
    #    DETALLE DE NOMINACIONES POR PELÍCULA (RESUMEN LATERAL)
    # ============================================================

    st.markdown("### 🎯 Detalle de nominaciones por película")

    if ff_base.empty:
        st.info("No hay películas con nominaciones para este año.")
    else:
        films_year = (
            ff_base["Film"]
            .dropna()
        ...
        films_year = sorted(f for f in films_year if f.strip())

        if not films_year:
            st.info("No hay películas con nombre válido en este año.")
        else:
            selected_film = st.selectbox(
                "Elegir película para ver detalle de nominaciones",
                options=films_year,
                key="osc_film_detail",
            )

            df_film = ff_base[ff_base["Film"] == selected_film].copy()
            if df_film.empty:
                st.info("No encontré nominaciones para esa película (con los filtros actuales).")
            else:
                n_noms = len(df_film)
                n_wins = int(df_film["IsWinner"].sum())

                cold1, cold2 = st.columns(2)
                with cold1:
                    st.metric("Nominaciones totales", n_noms)
                with cold2:
                    st.metric("Premios ganados", n_wins)

                detail = df_film[
                    ["CategoryNorm", "Nominee", "IsWinner"]
                ].copy()
                ...
                detail["Ganó"] = detail["Ganó"].map({True: "🏆", False: "—"})
