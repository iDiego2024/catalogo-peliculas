...
# ===================== Versión y changelog =====================
APP_VERSION = "1.2.0"  # <- súbela cuando publiques cambios

CHANGELOG = {
    "1.2.0": [
        "Bugfix: cálculo de Diff (Mi − IMDb) corregido en 'Películas infravaloradas'.",
        "AFI: eliminación del duplicado de 'Tootsie' (rank 91).",
        "Rendimiento: batching por página para TMDb y toggle para providers.",
        "Rendimiento: TTL en caches de APIs (TMDb/OMDb/YouTube).",
        "OMDb: backoff suave opcional en lotes grandes.",
        "UX: reset de página de galería al cambiar filtros/búsqueda.",
        "UX: persistencia de filtros en URL (experimental_* query params).",
        "Accesibilidad: alt-text consistente en pósters.",
        "Óscar: cruce con catálogo tolerante a desfaces de año (±1).",
        "Mensajes de secrets claros si faltan API keys."
    ],
    "1.1.4": [
        "Óscar: ahora se usa Oscar_Data_1927_today.xlsx como fuente principal.",
        "Óscar: filtros basados en el año de la película (Year), no en la ceremonia.",
        "Óscar: se elimina la gráfica de 'Análisis por categoría (ganadores)'.",
        "Óscar: nuevas tablas de estadísticas por categoría usando Film, Name y Nominee.",
        "Óscar: rankings por personas/entidades basados en Name/Nominee en lugar de NomineeIds.",
    ],
    "1.1.3": [
...
AFI_LIST = [
    ...
    # Rank 91 corregido: se elimina el duplicado de "Tootsie".
...
# En caso de que alguien reintroduzca el duplicado sin querer:
AFI_LIST = [x for i, x in enumerate(AFI_LIST) if not (x.get("Rank")==91 and x.get("Title")=="Tootsie")]
...
@st.cache_data
def _read_csv_any(file_path_or_buffer):
    """Lee CSV desde path o UploadedFile, devolviendo DataFrame."""
...
def load_data(file_path_or_buffer):
    df = _read_csv_any(file_path_or_buffer)
...
def get_youtube_trailer_url(title, year=None, language_hint="es"):
    ...
    q = f"{title} trailer {language_hint}"
    try:
...
@st.cache_data(ttl=24*60*60)
def get_omdb_awards(title, year=None, _sleep=0.0):
    """Info de premios desde OMDb (texto + parseo básico). _sleep para backoff suave."""
    ...
    def _query(params, sleep_time=0.0):
        if sleep_time > 0:
    ...
    for t in [raw_title, simple_title]:
        ...
        if year_int:
            params["y"] = year_int
        candidate = _query(params, sleep_time=_sleep)
    ...
    if data is None:
        ...
        if year_int:
            params["y"] = year_int
        search = _query(params, sleep_time=_sleep)
        if search and "error" not in search and "Search" in search:
            best = search["Search"][0]
            imdb_id = best.get("imdbID")
            if imdb_id:
                data = _query({"apikey": api_key, "i": imdb_id}, sleep_time=_sleep)
...
def compute_awards_table(df_basic):
    ...
    rows = []
    n = len(df_basic)
    # Backoff suave si el lote es grande
    sleep = 0.3 if n > 50 else 0.0
    for _, r in df_basic.iterrows():
        title = r.get("Title")
        year = r.get("Year")
        awards = get_omdb_awards(title, year, _sleep=sleep)
...
# ===================== ÓSCAR: carga y helpers (Oscar_Data_1927_today.xlsx) =====================

@st.cache_data
def load_full_data(path="Oscar_Data_1927_today.xlsx"):
    """
    Carga robusta del dataset de premios Óscar desde el Excel Oscar_Data_1927_today.xlsx.
    Campos esperados (nombres aproximados, se buscan de forma case-insensitive):

      - Año de la película:  Year / year
      - Categoría canónica:  CanonicalCategory / canonical_category / CanonCat / Category
      - Película:            Film / film / Title / title
      - Persona:             Name / name
      - Entidad/estudio:     Nominee / nominee
      - Ganador:             Winner / winner / IsWinner

    Además se añaden columnas:
      - YearInt: año numérico de la película
      - CanonCat: categoría canónica
      - IsWinner: booleano de ganador
      - NormFilm: título normalizado para cruce con tu catálogo
    """
    # 1) Intentamos leer como Excel (nuevo flujo)
    try:
        dff = pd.read_excel(path)
    except Exception:
        # Fallback: compatibilidad con CSV si hiciera falta
        try:
            dff = pd.read_csv(path, sep=None, engine="python", on_bad_lines="skip")
        except Exception:
            dff = pd.read_csv(path, sep="\t", on_bad_lines="skip")

    # 2) Normalizamos encabezados
    dff.columns = [str(c).strip() for c in dff.columns]
    col_map = {c.lower(): c for c in dff.columns}
    idx = dff.index
    ...
    if year_col:
        yr = pd.to_numeric(dff[year_col], errors="coerce")
        dff["YearInt"] = yr.fillna(-1).astype(int)
    else:
        dff["YearInt"] = pd.Series([-1] * len(dff), index=idx, dtype=int)

    # --- Categoría canónica ---
    canon_col = get_col("CanonicalCategory", "canonical_category", "CanonCat", "Category")
    if canon_col:
        dff["CanonCat"] = dff[canon_col].astype(str)
    else:
        dff["CanonCat"] = ""

    # --- Ganador ---
    winner_col = get_col("Winner", "IsWinner")
    if winner_col:
        dff["IsWinner"] = (
            dff[winner_col]
            .astype(str)
            .str.lower()
            .isin(["1", "true", "yes", "winner", "ganador", "ganadora", "y"])
        )
    else:
        dff["IsWinner"] = False

    # --- Película ---
    film_col = get_col("Film", "Title", "film", "title")
    if film_col:
        dff["Film"] = dff[film_col].astype(str)
    else:
        dff["Film"] = ""
    ...
    if name_col:
        dff["Name"] = dff[name_col].astype(str)
    else:
        dff["Name"] = ""

    # --- Entidad / estudio (Nominee) ---
    nominee_col = get_col("Nominee")
    if nominee_col:
        dff["Nominee"] = dff[nominee_col].astype(str)
    else:
        dff["Nominee"] = ""
...
def attach_catalog_to_full(osc_df, my_catalog_df):
    """
    Enlaza cada fila de datos de Óscar con tu catálogo (por título normalizado + año, tolerando desfase ±1).
    ...
    if merged is None:
        merged = out.copy()
        merged["URL"] = None
        merged["Your Rating"] = None
        merged["IMDb Rating"] = None
...
# ----------------- Avisos de secrets -----------------
with st.sidebar.expander("🔐 Estado de integraciones", expanded=False):
    if TMDB_API_KEY is None:
        st.warning("TMDb desactivado: falta TMDB_API_KEY en secrets.")
    else:
        st.caption("TMDb: ✅ configurado")
    if YOUTUBE_API_KEY is None:
        st.info("Tráilers desactivados: falta YOUTUBE_API_KEY.")
    else:
        st.caption("YouTube: ✅ configurado")
    if st.secrets.get("OMDB_API_KEY", None) is None:
        st.info("OMDb desactivado: falta OMDB_API_KEY.")
    else:
        st.caption("OMDb: ✅ configurado")
...
with st.sidebar.expander("⚡ Optimización TMDb (rendimiento)", expanded=False):
    fetch_providers = st.checkbox(
        "Consultar plataformas de streaming (TMDb)",
        value=True,
        help="Desactívalo si notas lentitud en páginas con muchas tarjetas."
    )
...
# ----------------- Persistencia por URL (leer antes de crear widgets) -----------------
...
# Derivar defaults para sliders desde datos
...
# Defaults provenientes de URL si existen
def _qp_int(name, default):
    try:
        return int(qp.get(name, [default])[0])
    except Exception:
        return default
...
def _qp_text(name, default=""):
    return qp.get(name, [default])[0]

y0_def = _qp_int("y0", min_year)
y1_def = _qp_int("y1", max_year)
r0_def = _qp_int("r0", min_rating)
r1_def = _qp_int("r1", max_rating)
g_def = _qp_csv("g")
d_def = _qp_csv("d")
q_def = _qp_text("q", "")
...
# Widgets con defaults de la URL (cuando existan)
if df["Year"].notna().any():
    year_range = st.sidebar.slider(
        "Rango de años", min_year, max_year, (max(min_year, y0_def), min(max_year, y1_def))
    )
else:
    year_range = (0, 9999)
...
if df["Your Rating"].notna().any():
    rating_range = st.sidebar.slider(
        "Mi nota (Your Rating)", min_rating, max_rating, (max(min_rating, r0_def), min(max_rating, r1_def))
    )
else:
    rating_range = (0, 10)
...
selected_genres = st.sidebar.multiselect(
    ...
    options=all_genres,
    default=[g for g in g_def if g in all_genres]
)

selected_directors = st.sidebar.multiselect(
    "Directores",
    options=all_directors,
    default=[d for d in d_def if d in all_directors]
)
...
try:
    st.experimental_set_query_params(
        y0=str(year_range[0]), y1=str(year_range[1]),
        r0=str(rating_range[0]), r1=str(rating_range[1]),
        g=",".join(selected_genres),
        d=",".join(selected_directors),
        q=""
    )
except Exception:
    pass
...
# ---------- Reset de página de galería cuando cambien filtros/búsqueda ----------
sig = (
    tuple(year_range), tuple(rating_range),
    tuple(sorted(selected_genres)), tuple(sorted(selected_directors)),
    search_query
)
if st.session_state.get("gallery_signature") != sig:
    st.session_state.gallery_current_page = 1
st.session_state.gallery_signature = sig
...
with tab_catalog:
    ...
    if total_pelis == 0:
        ...
        # ---------- Batching TMDb por página ----------
        page_tmdb_cache = {}
        if use_tmdb_gallery:
            for _, row in page_df.iterrows():
                t, y = row.get("Title"), row.get("Year")
                page_tmdb_cache[(t, y)] = get_tmdb_basic_info(t, y)

        provider_cache = {}
        if use_tmdb_gallery and fetch_providers:
            for (t, y), info in page_tmdb_cache.items():
                if info and info.get("id"):
                    tmdb_id = info["id"]
                    provider_cache[tmdb_id] = get_tmdb_providers(tmdb_id, country="CL")
        ...
        for _, row in page_df.iterrows():
            ...
            if use_tmdb_gallery:
                tmdb_info = page_tmdb_cache.get((titulo, year))
                if tmdb_info:
                    poster_url = tmdb_info.get("poster_url")
                    tmdb_rating = tmdb_info.get("vote_average")
                    tmdb_id = tmdb_info.get("id")
                    availability = provider_cache.get(tmdb_id) if fetch_providers else None
                else:
                    poster_url = None
                    tmdb_rating = None
                    availability = None
            ...
            if isinstance(poster_url, str) and poster_url:
                alt_text = f"Póster de {titulo}{' ('+fmt_year(year)+')' if pd.notna(year) else ''}"
                poster_html = f"""
...
"""
                ...
                poster_html = """
<div class="movie-poster-frame">
  <div class="movie-poster-placeholder" aria-label="Sin póster disponible">
    ...
    with st.expander("Películas que puntúo muy alto y IMDb no tanto", expanded=False):
        if "Your Rating" in df.columns and "IMDb Rating" in df.columns:
            ...
            if diff_df.empty:
                ...
                # BUGFIX: antes restaba contra df["IMDb Rating"] (posible desalineación)
                diff_df["Diff"] = diff_df["Your Rating"] - diff_df["IMDb Rating"]
...
# ============================================================
#                     TAB 4: PREMIOS ÓSCAR (FULL DATA XLSX)
# ============================================================

with tab_awards:
    st.markdown("## 🏆 Premios de la Academia (Oscar_Data_1927_today.xlsx)")

    osc = load_full_data("Oscar_Data_1927_today.xlsx")
    if osc.empty:
        st.info("No pude cargar 'Oscar_Data_1927_today.xlsx'. Asegúrate de incluirlo en el repo.")
        st.stop()

    # Enlaza con tu catálogo (con tolerancia ±1 año)
    ...
    valid_years = osc_x.loc[osc_x["YearInt"] > 0, "YearInt"]
    if valid_years.empty:
        min_year_osc, max_year_osc = 1927, 2025
    else:
        min_year_osc, max_year_osc = int(valid_years.min()), int(valid_years.max())

    with colf1:
        year_range_osc = st.slider(
            "Año de la película",
            min_year_osc,
            max_year_osc,
            (min_year_osc, max_year_osc),
        )
    ...
    with colf3:
        q_aw = st.text_input(
            "Buscar (categoría / persona / película / entidad)",
            placeholder="Ej: 'BEST PICTURE' o 'Spielberg' o 'Paramount'"
        )
    ...
    if q_aw:
        ...
        mask = (
            ...
            ff.get("Name", "").astype(str).str.lower().str.contains(q, na=False) |
            ff.get("Nominee", "").astype(str).str.lower().str.contains(q, na=False)
        )
        ff = ff[mask]
    ...
    st.caption(
        "Se usa **Oscar_Data_1927_today.xlsx**. El resaltado **verde** marca a los ganadores. "
        "Las estadísticas usan los campos **Film**, **Name** y **Nominee**."
    )

    # --------- Vista por año: categorías, nominados y ganadores ----------
    ...
    if years_sorted:
        y_pick = st.selectbox(
            "Elige año de la película",
            options=years_sorted,
            index=len(years_sorted) - 1,
            key="aw_select_year_full"
        )
        table_year = ff[ff["YearInt"] == y_pick].copy().sort_values(
            ["CanonCat", "IsWinner", "Film", "Name", "Nominee"],
    ...
    if table_year.empty:
        ...
        pretty = table_year[[
            "CanonCat", "Film", "Name", "Nominee", "YearInt", "IsWinner",
    ...
    # --------- Rankings (por nominaciones en el rango seleccionado) ----------
    st.markdown("### 🥇 Rankings en el rango seleccionado (nominaciones)")

    nom_ff = ff.copy()

    colr1, colr2 = st.columns(2)

    # Top películas por nº de nominaciones
    with colr1:
        top_films = (
        ...
        if not top_films.empty:
            ...
            st.markdown("**Top películas nominadas en el rango**")
            st.dataframe(tf_disp, use_container_width=True, hide_index=True)
        else:
            st.write("Sin datos de películas para este rango.")

    # Top personas/entidades (Name / Nominee) por nº de nominaciones
    with colr2:
        st.markdown("**Top personas / entidades nominadas en el rango**")

        series_list = []
        if "Name" in nom_ff.columns:
            series_list.append(nom_ff["Name"].dropna().astype(str))
        if "Nominee" in nom_ff.columns:
            series_list.append(nom_ff["Nominee"].dropna().astype(str))

        if series_list:
            all_entities = pd.concat(series_list)
            all_entities = all_entities[all_entities.str.strip() != ""]
            if not all_entities.empty:
                ents = (
                    all_entities.value_counts()
                    .reset_index()
                    .head(20)
                )
                ents.columns = ["Nombre / Entidad", "Nominaciones"]
                st.dataframe(ents, use_container_width=True, hide_index=True)
            else:
                st.write("No hay suficientes nombres/entidades no vacíos.")
        else:
            st.write("No encontré columnas 'Name' ni 'Nominee' en los datos.")

    # --------- Estadísticas por categoría (ganadores) con Film / Name / Nominee ----------
    st.markdown("### 📊 Estadísticas por categoría (ganadores)")

    if all_cats:
        default_idx = all_cats.index("BEST PICTURE") if "BEST PICTURE" in all_cats else 0
        cat_for_stats = st.selectbox(
            "Elige una categoría para ver sus ganadores",
            options=all_cats,
            index=default_idx,
            key="aw_stats_cat_full",
        )
        fa = ff[(ff["CanonCat"] == cat_for_stats) & (ff["IsWinner"])].copy()

        if fa.empty:
            st.info("No hay ganadores para esa categoría en el rango actual.")
        else:
            colc1, colc2, colc3 = st.columns(3)

            # Top películas ganadoras en esa categoría
            with colc1:
                top_f_cat = (
                    fa.groupby(["Film", "YearInt"])
                    .size()
                    .reset_index(name="Premios")
                    .sort_values(["Premios", "Film"], ascending=[False, True])
                    .head(15)
                )
                top_f_cat["YearInt"] = top_f_cat["YearInt"].apply(
                    lambda v: "" if v == -1 or pd.isna(v) else str(int(v))
                )
                st.markdown("**Top películas ganadoras**")
                st.dataframe(
                    top_f_cat.rename(columns={"Film": "Película", "YearInt": "Año"}),
                    use_container_width=True,
                    hide_index=True
                )

            # Top personas (Name) ganadoras
            with colc2:
                if "Name" in fa.columns:
                    top_n_cat = (
                        fa[fa["Name"].astype(str).str.strip() != ""]
                        .groupby("Name")
                        .size()
                        .reset_index(name="Premios")
                        .sort_values(["Premios", "Name"], ascending=[False, True])
                        .head(15)
                    )
                    if not top_n_cat.empty:
                        st.markdown("**Top personas (Name)**")
                        st.dataframe(
                            top_n_cat.rename(columns={"Name": "Persona"}),
                            use_container_width=True,
                            hide_index=True
                        )
                    else:
                        st.write("No hay datos de Name para esta categoría.")
                else:
                    st.write("La columna 'Name' no está disponible en los datos.")

            # Top entidades (Nominee) ganadoras
            with colc3:
                if "Nominee" in fa.columns:
                    top_e_cat = (
                        fa[fa["Nominee"].astype(str).str.strip() != ""]
                        .groupby("Nominee")
                        .size()
                        .reset_index(name="Premios")
                        .sort_values(["Premios", "Nominee"], ascending=[False, True])
                        .head(15)
                    )
                    if not top_e_cat.empty:
                        st.markdown("**Top entidades (Nominee)**")
                        st.dataframe(
                            top_e_cat.rename(columns={"Nominee": "Entidad"}),
                            use_container_width=True,
                            hide_index=True
                        )
                    else:
                        st.write("No hay datos de Nominee para esta categoría.")
                else:
                    st.write("La columna 'Nominee' no está disponible en los datos.")
    else:
        st.info("No hay categorías disponibles en los datos.")

    # --------- (Opcional) Detalle de nominaciones del año elegido ----------
    with st.expander("🔎 (Opcional) Ver también nominaciones del año elegido"):
        if y_pick is None:
            st.info("Elige un año de película para ver nominaciones.")
        else:
            nom_year = osc_x[osc_x["YearInt"] == y_pick].copy()
            if nom_year.empty:
                st.write("No hay nominaciones en el archivo para ese año.")
            else:
                nom_year = nom_year[["CanonCat", "Film", "Name", "Nominee", "IsWinner"]].sort_values(
                    ["CanonCat", "IsWinner", "Film", "Name", "Nominee"],
                    ascending=[True, False, True, True, True]
                )
