...
def load_oscar_data_from_excel(path_xlsx="Oscar_Data_1927_today.xlsx"):
    """
    Carga el Excel Oscar_Data_1927_today.xlsx y normaliza columnas para la app.
    Esperamos algo tipo:
      - Year_Film  (año de la película)  -> FilmYear
      - Year_Award (año de la ceremonia) -> AwardYear
      - Category   -> CategoryRaw
      - Name       -> PersonName (actor, actriz, técnico, etc.)
      - Film       -> Film
    ...
    # Año de película
    if "year_film" in cols:
        film_year_col = cols["year_film"]
    elif "year film" in cols:
        film_year_col = cols["year film"]
    elif "year" in cols:
        film_year_col = cols["year"]
    else:
        film_year_col = None
    ...
    # Año de ceremonia
    if "year_award" in cols:
        award_year_col = cols["year_award"]
    elif "year award" in cols:
        award_year_col = cols["year award"]
    elif "ceremony" in cols:
        award_year_col = cols["ceremony"]
    else:
        award_year_col = None
    ...
    # Nombre de persona / entidad
    if "name" in cols:
        name_col = cols["name"]
    elif "nominee" in cols:
        name_col = cols["nominee"]
    else:
        name_col = None
    ...
    if "winner" in cols:
        ...
        if pd.api.types.is_bool_dtype(w):
            ...
            is_winner = (
                ...
                .str.strip()
                .str.lower()
                .isin(["true", "1", "yes", "winner", "ganador", "ganadora"])
            )
    else:
        is_winner = pd.Series([False] * len(raw))
    ...
    df_osc = pd.DataFrame(
        {
            ...
            "IsWinner": is_winner.astype(bool),
        }
    )
    ...
    # Categoría "bonita" en mayúsculas consistentes
    df_osc["Category"] = (
        df_osc["CategoryRaw"]
        .astype(str)
        .str.strip()
    ...
    # Normalización de títulos para cruce con tu catálogo
...
def attach_catalog_to_oscar(osc_df, my_catalog_df):
    """
    Enlaza cada fila del Excel de Óscar con tu catálogo, por título normalizado + año de película.
    Añade:
      - InMyCatalog
      - MyRating
      - MyIMDb
    ...
    merged = merged.drop(columns=["NormTitle", "Your Rating", "IMDb Rating", "YearInt"], errors="ignore")
    return merged
...
def build_oscar_movie_card_html(
    ...
    Marca ganador, catálogo, IMDb, streaming, personas nominadas, etc.
    """
    # Póster y TMDb
    poster_url = None
    tmdb_rating = None
    ...
    # Rating base para color
    base_rating = my_rating if pd.notna(my_rating) else my_imdb
    border_color, glow_color = get_rating_colors(base_rating)

    # Si ganó en la categoría, borde verde llamativo
    if is_winner_cat:
        border_color = "#22c55e"
    ...
    imdb_link_html = (
        f'<a href="{imdb_url}" target="_blank">Ver en mi ficha de IMDb</a><br>'
        if isinstance(imdb_url, str) and imdb_url.startswith("http")
        else ""
    )
    reseñas_html = (
        f'<a href="{reseñas_url}" target="_blank">Reseñas en español</a><br>'
        if reseñas_url
        else ""
    )
    ...
    streaming_html = f"Streaming (CL): {platforms_str}<br>"
    if streaming_link:
        streaming_html += f'<a href="{streaming_link}" target="_blank">Ver streaming en TMDb (CL)</a><br>'
    ...
    if in_my_catalog:
        ...
        catalog_badge = (
            "<div style='margin-top:6px;display:flex;flex-wrap:wrap;gap:4px;'>"
            f"<span style='background:rgba(234,179,8,0.16);border-radius:999px;"
            "padding:2px 8px;font-size:0.7rem;text-transform:uppercase;"
            "letter-spacing:0.12em;border:1px solid #facc15;color:#fef9c3;'>"
            f"En mi catálogo · Mi nota: {rating_txt}</span></div>"
        )

    # Personas nominadas (actores, directores, etc.)
    people_html = ""
    clean_people = [p for p in people_list if isinstance(p, str) and p.strip() and p.strip() != "nan"]
    if clean_people:
        chips = []
...
<div class="movie-card movie-card-grid"
  ...
  <div class="movie-sub">
    ...
    {reseñas_html}
    {streaming_html}
    <br><span style="font-size:0.78rem;color:#9ca3af;">Premio en esta categoría:</span><br>
    <span style="font-size:0.82rem;font-weight:500;">{category}</span>
    {winner_badge}
    {catalog_badge}
    {people_html}
  </div>
...
with tab_awards:
    ...
    # Años de película (línea de tiempo horizontal)
    with col_years:
        ...
        default_year = years_sorted[-1] if years_sorted else None
    ...
    # Quitamos filas sin película (alguna fila rara)
    ...
    with col_m4:
        st.metric("Premios (filas Winner=TRUE)", int(ff["IsWinner"].sum()))
    ...
    st.caption(
        ...
        "En modo ganadores se muestran sólo las películas que ganaron al menos un premio en el año."
    )

    # --- Información de TMDb por película del año seleccionado ---
    # (para no llamar a la API demasiadas veces reutilizamos un pequeño cache en memoria)
    tmdb_cache = {}
    providers_cache = {}
    ...
    if ff.empty:
        ...
        gallery_base_df = pd.DataFrame()
    else:
        # =========================
        #   MODO: TODOS LOS NOMINADOS (CON CATEGORÍAS)
        # =========================
        if not show_only_winners:
            ...
            for cat in cats_in_year:
                ...
                st.markdown(
                    f"<div style='margin-top:22px;margin-bottom:10px;font-size:1.05rem;"
                    f"font-weight:700;text-transform:uppercase;letter-spacing:0.12em;'>"
                    f"🎞️ {cat}</div>",
                    unsafe_allow_html=True,
                )

                # Agrupamos por película dentro de la categoría
                grouped = (
                    cat_rows
                    .groupby(["Film", "FilmYear"], dropna=False)
                )
        ...
        # =========================
            ...
            # Agrupamos por película en todo el año
            ...
            for (film_title, film_year), g in grouped_all:
                ...
                # Lista de categorías ganadas y nom/gan
                cats_won = (
                    wins["Category"]
                    .dropna()
                    .unique()
                ...
                # Recogemos nombres de personas que ganan
    ...
    if ff.empty:
        st.info("No hay películas para este año con los filtros actuales.")
    else:
        films_in_year = (
            ff["Film"]
            .dropna()
        ...
        film_rows = ff[ff["Film"] == sel_film].copy()
        if film_rows.empty:
            st.info("No encontré filas para esa película (¿filtros demasiado restrictivos?).")
        else:
            film_year_val = film_rows["FilmYear"].dropna().iloc[0] if film_rows["FilmYear"].notna().any() else None
            ...
            my_rating = film_rows["MyRating"].dropna().iloc[0] if film_rows["MyRating"].notna().any() else None
            my_imdb = film_rows["MyIMDb"].dropna().iloc[0] if film_rows["MyIMDb"].notna().any() else None
            imdb_url = film_rows["CatalogURL"].dropna().iloc[0] if film_rows["CatalogURL"].notna().any() else None
            ...
            awards_badge = (
                f"<span style='background:rgba(34,197,94,0.18);border-radius:999px;"
                f"padding:4px 10px;font-size:0.78rem;text-transform:uppercase;"
                f"letter-spacing:0.12em;border:1px solid #22c55e;color:#bbf7d0;'>🏆 {n_wins} premio(s)</span>"
            )
            noms_badge = (
                f"<span style='background:rgba(148,163,184,0.18);border-radius:999px;"
                f"padding:4px 10px;font-size:0.78rem;text-transform:uppercase;"
                f"letter-spacing:0.12em;border:1px solid rgba(148,163,184,0.85);color:#e5e7eb;'>"
            ...
            # Lista de nominaciones con su estado
            detalle_rows = []
            for _, r in film_rows.sort_values(["Category", "IsWinner"], ascending=[True, False]).iterrows():
                cat = str(r["Category"])
                person = str(r["PersonName"]) if pd.notna(r["PersonName"]) else ""
                label = f"{cat} · {person}" if person else cat
                status = "GANÓ" if bool(r["IsWinner"]) else "NOMINADA"
                if r["IsWinner"]:
                    status_chip = (
                        "<span style='background:rgba(34,197,94,0.20);border-radius:999px;"
...
<div style="display:flex;justify-content:space-between;align-items:center;
  ...
  <div style="font-size:0.86rem;color:#e5e7eb;">{label}</div>
  <div>{status_chip}</div>
</div>
"""
                )
...
<div style="border-radius:22px;border:1px solid rgba(148,163,184,0.45);
  ...
  <div style="display:flex;flex-wrap:wrap;gap:20px;">
    ...
    <div style="flex:1 1 320px;margin-top:4px;">
      <div style="font-size:0.82rem;letter-spacing:0.18em;text-transform:uppercase;color:#9ca3af;margin-bottom:4px;">
