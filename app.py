...
# ===================== Versión y changelog =====================
APP_VERSION = "1.1.7"  # <- Actualizada

CHANGELOG = {
    "1.1.7": [
        "Óscar: Corrección crítica de renderizado HTML en tarjetas de galería (div visible).",
        "Óscar: Corrección de codificación de caracteres (tildes/ñ) en nombres de nominados.",
        "Refactorización: Limpieza de funciones duplicadas para mejorar rendimiento.",
    ],
    "1.1.6": [
...
# ... [Omitido por brevedad: Tabs 1, 2, 3 sin cambios] ...
# (Asumimos que el código de los tabs 1, 2 y 3 es idéntico a la versión anterior.
#  Solo se incluye el código relevante para arreglar el Tab 4).

# ============================================================
#                     TAB 4: PREMIOS ÓSCAR
# ============================================================

# ---------- Funciones globales para carga de Óscar (MOVIDAS FUERA DE TABS) ----------
...
def fix_mojibake(text):
    """Intenta corregir texto mal codificado (ej: TimothÃ©e -> Timothée)."""
    if not isinstance(text, str):
        return text
    try:
        # Intento de conversión común: Latin-1 interpretado como UTF-8
...
def load_oscar_data_from_excel(path_xlsx="Oscar_Data_1927_today.xlsx"):
    ...
    # Categoría
    cat_col = None
    for key in ["category", "award", "award category"]:
        if key in cols_lower:
            cat_col = cols_lower[key]
            break
    if cat_col is None:
        cat_col = list(raw.columns)[0] if len(raw.columns) > 0 else "Category"

    # Persona / entidad
    name_col = None
    for key in ["name", "nominee", "primary nominee"]:
        if key in cols_lower:
    ...
    # Película
    film_col = None
    for key in ["film", "movie", "film title"]:
        if key in cols_lower:
    ...
    df_osc = pd.DataFrame(
        {
            ...
            "CategoryRaw": raw[cat_col].astype(str) if cat_col in raw.columns else "",
    ...
    # Intento de corrección de Mojibake (TimothÃ©e -> Timothée)
    if "PersonName" in df_osc.columns:
        df_osc["PersonName"] = df_osc["PersonName"].apply(fix_mojibake)

    return df_osc

def attach_catalog_to_oscars(osc_df, my_catalog_df):
    """
    Enlaza el dataset de Óscar con tu catálogo IMDb.
    """
...
def build_oscar_movie_card_html(
    ...
    """
    Construye el HTML para la tarjeta de la galería de Óscar.
    """
    # --- TMDb info ---
    ...
    imdb_link_html = f'<a href="{imdb_url}" target="_blank">Ver en mi ficha de IMDb</a>' if isinstance(imdb_url, str) and imdb_url.startswith("http") else ""
    ...
    if providers_info:
        ...
        streaming_html = (
            ...
            if streaming_link else f"Streaming (CL): {platforms_str}"
        )
    else:
    ...
    # --- Badges ---
    winner_badge = (
        "<span style='background:rgba(34,197,94,0.20);border-radius:999px;padding:3px 10px;font-size:0.72rem;text-transform:uppercase;letter-spacing:0.12em;border:1px solid #22c55e;color:#bbf7d0;'>WINNER 🏆</span>"
        if is_winner_in_this_context else ""
    )
    
    rating_txt = f"{float(my_rating):.1f}" if pd.notna(my_rating) else "?"
    catalog_badge = (
        f"<span style='background:rgba(234,179,8,0.16);border-radius:999px;padding:3px 10px;font-size:0.72rem;text-transform:uppercase;letter-spacing:0.12em;border:1px solid #facc15;color:#fef9c3;'>En mi catálogo · Mi nota: {rating_txt}</span>"
        if in_my_catalog else ""
    )
    
    badges_row = ""
    if winner_badge or catalog_badge:
        badges_row = f"<div style='margin-top:8px;display:flex;flex-wrap:wrap;gap:8px;'>{winner_badge}{catalog_badge}</div>"

    # --- Personas (FIXED: Construcción segura para evitar markdown injection) ---
    people_html = ""
    names = []
    ...
    if names:
        # Importante: No dejar saltos de línea dentro del string HTML para evitar que el markdown lo parsee mal
        people_html = f"<div style='margin-top:8px;font-size:0.78rem;color:#e5e7eb;'>Nominado(s): {', '.join(names)}</div>"

    # --- Ratings ---
    ratings_parts = []
    if tmdb_rating is not None and not pd.isna(tmdb_rating):
        ratings_parts.append(f"TMDb: {float(tmdb_rating):.1f}")
    if my_imdb is not None and not pd.isna(my_imdb):
        ratings_parts.append(f"IMDb: {float(my_imdb):.1f}")
    ratings_html = "<br>".join(ratings_parts)

    # --- Poster ---
    if poster_url:
        poster_html = f"<div class='movie-poster-frame'><img src='{poster_url}' alt='{film_title}' class='movie-poster-img' /></div>"
    else:
        poster_html = "<div class='movie-poster-frame'><div class='movie-poster-placeholder'><div class='film-reel-icon'>🎞️</div><div class='film-reel-text'>Sin póster</div></div></div>"

    category_block = f"<span style='font-size:0.78rem;color:#9ca3af;'>Categoría(s):</span><br><span style='font-size:0.82rem;font-weight:500;'>{category_text}</span>"
    
    # --- Bloque de enlaces ---
    links_parts = []
    if reseñas_html: links_parts.append(reseñas_html)
    if imdb_link_html: links_parts.append(imdb_link_html)
    links_block = "<br>".join(links_parts)

    # Construcción final sin indentación interna para las variables HTML
    card_html = f"""
<div class="movie-card movie-card-grid" style="border-color:{border_color}; box-shadow:0 0 0 1px rgba(15,23,42,0.9), 0 0 26px {glow_color};">
  {poster_html}
...
with tab_awards:
    ...
    # Cache ligero para TMDb y proveedores (por año de vista)
    tmdb_cache = {}
    providers_cache = {}
