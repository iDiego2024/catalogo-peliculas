...
st.markdown(
    ...
    .main .block-container {{
        max-width: 1200px;
        padding-top: 3.2rem;   /* un poco más para que no se corte la cabecera */
        padding-bottom: 3rem;
    }}
...
with tab_catalog:
    ...
    if total_pelis == 0:
        ...
        # Mostramos en una cuadrícula de 6 columnas
        ...
        for i, (_, row) in enumerate(page_df.iterrows()):
            ...
            with col:
                ...
                # Obtenemos póster desde TMDb
                tmdb_info = get_tmdb_basic_info(titulo, year)
                poster_url = tmdb_info.get("poster_url") if tmdb_info else None

                # Precargamos para evitar fallo silencioso de Streamlit
                if isinstance(poster_url, str) and poster_url:
                    try:
